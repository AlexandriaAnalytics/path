                    ...
                    ->form(function (CandidateRecord $record) {
                            ...
                            foreach ($questions as $index => $question) {
                                ...
                                if (!$record->result || ($record->result && !$question->evaluation)) {
                                    ...
                                    $schema[] = TiptapEditor::make('final' . $index)
                                        ...
                                        ->disabled()
                                        ->live()
                                        ->visible(function () use ($index, $questions) {
                                            return $index + 1 == count($questions);
                                        });
                        ...
                        return [
                            Wizard::make($steps)
                                ...
                                ->columnSpanFull()
                                ->disabled(function (CandidateRecord $record) {
                    ...
                    ->action(function (array $data, CandidateRecord $record) {
                        if (!$record->result) {
                            ...
                            foreach ($questions as $index => $question) {
                                foreach ($question['question_type'] as $indice => $type) {
                                    if ($question->evaluation) {
                                        CandidateAnswer::where('question_id', $question['question_ids'][$indice])->where('candidate_id', $record->candidate_id)->delete();
                                    }
